...
class ViT(nn.Module):
    def __init__(self, seq_len, patch_size, n_channels, d_model, num_layers=12, n_head=12, d_ff=2048, dropout=0.1,
                 max_num_patches=224, pooling=False):
        ...
        self.embedding = nn.Linear(self.n_channels * self.patch_size, d_model, bias=False)
...
def vit(model_name, config: TransformerConfig):
    ...
    if scales == 'es':
        ...
        config.n_head = 16
    return ViT(seq_len=config.seq_len, patch_size=config.patch_size, n_channels=config.n_channels,
